...
`mimo_result` output levels are `compact` (default), `standard` (bounded operator diagnostics), and `full` (complete saved result, plan, verification evidence, safe job log, and diff). Compact results also expose reconciliation status and distinguish verified changed files from unverified candidates when detection is partial. Request `full` only for explicit manual troubleshooting; do not use it on normal heartbeat or callback paths.
