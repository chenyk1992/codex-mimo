...
6. Capture Git and scoped-workspace evidence, persist `<jobId>.execution.json`, then run reconciliation and ordered development acceptance when required.
...
The nine statuses are `queued`, `running`, `needs_input`, `blocked`, `stalled`, `completed`, `failed`, `cancelled`, and `timeout`. Only `queued` and `running` are active. Compose jobs additionally persist a deliverable `assessment` (`passed`, `needs_review`, or `failed`); compact callers must not interpret `status: completed` as approval when `assessment: needs_review`. `needs_input`, `blocked`, and `stalled` are paused results with no PID and may retain `sessionId` and checkpoint context.
...
`mimo_result` defaults to a compact delivery record: execution status, optional Compose deliverable assessment, changed files, compact verification/acceptance stage results, failure, report path, context-overhead metrics, and a bounded plan/review summary when applicable. Compact `mimo_status` also includes the assessment once known. Complete final text is not returned by default. `reportPath` is repository-relative when the artifact is inside the requested workspace. Default compact `mimo_result` JSON must not exceed 6,000 UTF-8 bytes. Acceptance failures include compact fields such as `failedStage`, failed command/tests, and a shortest-fix `suggestion`. A `slice_failed` root keeps its stable code while `failure.causes` begins with the actionable child error. When multiple failures occur, compact `failure.causes` keeps at most three entries; `standard` and `full` retain the complete list in `failureCauses`.

`contextOverhead` counts successful `mimo_status` and `mimo_result` calls for the logical parent/child job family, records the most recent compact payload size, counts Codex App Server callback attempts, and flags standard/full reads, `needs_input`, and resumes. `heartbeatCalls` is `null` because heartbeat-originated status requests are indistinguishable from manual status calls; `relaunchCount` is `null` because separate launches have no reliable correlation ID. `tracking` distinguishes complete, partial, and unavailable historical data. The per-job sidecar contains only timestamps, tool category, output level, and byte counts; it never stores request payloads, prompts, workspace paths, notification targets, secrets, or estimated tokens.
...
The workspace supervisor holds a physical-workspace process lock, adopts existing job/notification worker ownership after handoff, replaces crashed workers while queued/running jobs or unfinished deliveries remain, and stops when no work remains. Worker startup retries are bounded; a permanently unstartable queued job is failed through the normal transition/signal/notification path. Once MiMo execution ends, the worker persists execution evidence before reconciliation. A restarted worker with that evidence resumes reconciliation and never launches MiMo again. Reconciliation is attempted twice; if both attempts fail, the job remains `completed` with `reconciliation.status: "degraded"`, `errorCode: "reconciliation_failed"`, preserved changed-file/evidence paths, and an explicit warning instead of being misreported as an execution failure. Before execution evidence exists, ordinary PID/process-identity recovery rules still apply.
...
| Job paused with `acceptance_config_missing` | Call `mimo_resume` with `acceptance.build` / `acceptance.test` and optional `allowedPaths`; no prior session is required for this preflight pause |
