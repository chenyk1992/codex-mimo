...
`worktree` runs in a retained bridge-owned named worktree and can be resumed
without changing the control checkout. `merge` requires local `sourceRef` and
`targetRef` branches plus non-empty bounded `allowedPaths`; it pins both OIDs,
runs conflict resolution in a clean detached worktree, and publishes only a
new `codex-mimo/merge/<jobId>` integration branch. It never moves the target or
current branch. These worktrees provide cwd-relative containment, not an OS-
level sandbox.
